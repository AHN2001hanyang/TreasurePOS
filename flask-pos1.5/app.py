# -*- coding: utf-8 -*-
from flask import Flask, render_template, request, jsonify, send_file, redirect, url_for, make_response, after_this_request, Response, send_from_directory
...
TEXTS = {
    'ko': {
        ...
        'search_placeholder': '바코드 또는 상품명 입력', 'search_tip': '바코드 스캔, 직접 입력, 상품名搜索',
        'all': '전체', 'stock': '재고',
...
# ======== 持久化目录（关键改动） ========
def _compute_run_dir() -> str:
    env_dir = os.getenv('TREASUREPOS_DATA_DIR')
    if env_dir:
        return os.path.abspath(os.path.expanduser(env_dir))
    if os.name == 'nt':
        root = os.getenv('LOCALAPPDATA') or os.path.expanduser(r'~\AppData\Local')
        return os.path.join(root, 'TreasurePOS')
    if sys.platform == 'darwin':
        return os.path.expanduser('~/Library/Application Support/TreasurePOS')
    return os.path.expanduser('~/.local/share/treasurepos')
...
def _maybe_migrate_legacy():
    ...
    if (not os.path.exists(new_db)) and os.path.exists(legacy_db):
        try:
            ...
            pass
    # images
    legacy_images_dir = os.path.join(app.static_folder, 'images')
    if os.path.isdir(legacy_images_dir):
        try:
            for fn in os.listdir(legacy_images_dir):
                if re.fullmatch(r'[A-Za-z0-9_\-]+\.(?:jpe?g|png|webp)', fn, flags=re.I):
                    src = os.path.join(legacy_images_dir, fn)
                    dst = os.path.join(RUN_IMAGES, fn)
...
# Flask 仍指向 /static，但我们把 /static/images/* 映射到 RUN_IMAGES（前端无需修改）
@app.route('/static/images/<path:filename>')
def _serve_runtime_images(filename):
    # 安全白名单：仅允许我们定义的安全文件名
    safe = re.fullmatch(r'[A-Za-z0-9_\-]+\.(?:jpe?g|png|webp)', filename, flags=re.I)
    if not safe:
        return 'Not found', 404
    return send_from_directory(RUN_IMAGES, filename)

# ========= 路径与安全校验（改） =========
UPLOAD_FOLDER = RUN_UPLOADS
...
def is_safe_image_relpath(p: str) -> bool:
    ...
    if not SAFE_IMAGE_RE.fullmatch(p):
        return False
    # 从 'images/xxx' 提取文件名
    relname = p.split('/', 1)[1] if p.lower().startswith('images/') else p
    abs_path = os.path.normpath(os.path.join(IMAGE_FOLDER, relname))
    try:
        return os.path.commonpath([abs_path, IMAGE_FOLDER]) == IMAGE_FOLDER
    except Exception:
        return False

# ---- SQLite 连接助手（改为 RUN_DIR） ----
_DB_ROOT = RUN_DIR
...
def init_db():
    ...
    ''')
    try:
        c.execute('ALTER TABLE sales ADD COLUMN pay_type TEXT DEFAULT "cash"')
    except Exception:
        pass
    try:
        c.execute('ALTER TABLE sales ADD COLUMN total_int INTEGER DEFAULT 0')
    except Exception:
        pass
    ...
    ''')
    try:
        c.execute('ALTER TABLE sale_items ADD COLUMN price_int INTEGER DEFAULT 0')
    except Exception:
        pass
...
def export_sales():
    ...
    sql = f'''
        SELECT id, time, items,
               COALESCE(NULLIF(total_int,0), CAST(ROUND(total) AS INTEGER)) AS total,
               pay_type
...
def _save_and_validate_image(file_storage, save_name_base: str) -> str:
    """保存并验证图片，返回相对路径 'images/xxx.jpg'；失败返回空串"""
    try:
...
def add_item():
    ...
    conn = connect_db()
    c = conn.cursor()
    c.execute('SELECT 1 FROM items WHERE barcode=?', (barcode,))
    if c.fetchone():
        conn.close()
        return jsonify({'msg': 'Code already exists', 'field': 'barcode'}), 409
...
def edit_item(barcode):
    ...
    if request.content_type and request.content_type.startswith('multipart'):
        ...
        # 图片以“目标 code”命名，避免改码后文件名不一致
        barcode_safe_for_image = safe_filename(want_barcode)
        if image:
            image_path = _save_and_validate_image(image, barcode_safe_for_image)
        ...
        data = request.json or {}
        want_barcode = (data.get('barcode') or '').strip() or barcode
        name = data.get('name')
        price_i = to_amount_int(data.get('price', 0), 0)
        wholesale_i = to_amount_int(data.get('wholesale_price', 0), 0)
        qty = to_int(data.get('qty', 0), 0)
    ...
    # 若修改了 code，先查重
    if want_barcode != barcode:
        c.execute('SELECT 1 FROM items WHERE barcode=?', (want_barcode,))
        if c.fetchone():
            conn.close()
            return jsonify({'msg': 'Code already exists', 'field': 'barcode'}), 409

    # 原子更新：items +（若改码）sale_items/stock_log 同步
    try:
        c.execute('BEGIN IMMEDIATE')
        c.execute('''
            UPDATE items SET
                barcode=?,
                name=?, price=?, price_int=?,
                wholesale_price=?, wholesale_price_int=?,
                qty=?, category=?, size=?, status=?, image=?, discontinued_time=?
            WHERE barcode=?
        ''', (want_barcode,
              name, price_i, price_i,
              wholesale_i, wholesale_i,
              qty, category, size, status, image_path, discontinued_time,
              barcode))
        if want_barcode != barcode:
            c.execute('UPDATE sale_items SET barcode=? WHERE barcode=?', (want_barcode, barcode))
            c.execute('UPDATE stock_log SET barcode=? WHERE barcode=?', (want_barcode, barcode))
        conn.commit()
        conn.close()
        return jsonify({'msg': t('updated'), 'barcode': want_barcode})
    except sqlite3.IntegrityError:
        conn.rollback()
        conn.close()
        return jsonify({'msg': 'Code already exists', 'field': 'barcode'}), 409
...
def get_item(barcode):
    ...
    if row:
        ...
        return jsonify({
            ...
            'category': row[5], 'size': (row[6] or '').upper(), 'status': show_status, 'image': row[8], 'discontinued_time': row[9]
        })
    else:
...
def api_sales_top_items():
    ...
    conn.close()
    top = sorted(
        [{'barcode':k[0], 'name':k[1], 'category':k[2], 'size':(k[3] or '').upper(), 'count':v} for k,v in agg.items()],
...
def get_items():
    ...
    for r in rows:
        ...
        if r[4] == 0 and r[7] != '절판':
            show_status = '매진'
        item = {
            'barcode': r[0], 'name': r[1], 'price': int(r[2] or 0), 'wholesale_price': int(r[3] or 0), 'qty': r[4], 'category': r[5], 'size': (r[6] or '').upper(),
...
def search_items():
    ...
    for r in rows:
        ...
        items.append({
            ...
            'category': r[5], 'size': (r[6] or '').upper(), 'status': show_status, 'image': r[8], 'discontinued_time': r[9]
        })
...
def api_item_sales(barcode):
    ...
    try:
        ...
        if test is not None:
            c.execute('''
              SELECT s.id, s.time, si.qty, si.size, si.category,
                     COALESCE(NULLIF(si.price_int,0), CAST(ROUND(si.price) AS INTEGER)) AS price_i,
                     COALESCE(NULLIF(s.total_int,0), CAST(ROUND(s.total) AS INTEGER)) AS order_total_i
              FROM sales s
            ...
            conn.close()
            result = []
            for sale_id, time, qty, sz, cat, price_i, order_total_i in rows:
                result.append({
                    "sale_id": sale_id,
                    "time": time,
                    "qty": int(qty or 0),
        ...
        try:
            ...
            if barcode in items:
                ...
                all_sales.append({
                    ...
                    "size": (items[barcode].get('size', '') or '').upper(),
...
def api_sales():
    ...
    c.execute(f'''
        SELECT id, time, items,
               COALESCE(NULLIF(total_int,0), CAST(ROUND(total) AS INTEGER)) AS total_i,
               refunded, pay_type 
        FROM sales
        {where_sql}
...
def api_delete_sales():
    ...
    for sale_id in ids:
        c.execute('SELECT items, COALESCE(NULLIF(total_int,0), CAST(ROUND(total) AS INTEGER)) AS amount, refunded FROM sales WHERE id=?', (sale_id,))
        row = c.fetchone()
        if not row:
            not_found += 1
            continue
        ...
        try:
            ...
            # 先清 sale_items，再清 sales，避免“孤儿明细”
